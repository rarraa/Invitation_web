...
type TabId = "orders" | "templates" | "domains" | "settings";

// ---------------------------------------------------------------------------
// INITIAL DATA
// Sama seperti dashboard-pembeli: ini masih hardcode, nanti idealnya fetch dari
// API Laravel. Lihat catatan lengkap di bagian bawah file.
// ---------------------------------------------------------------------------
...
const TEMPLATE_CHOICES = [
  "The Ultramarine Royal Suite",
  "Pearl White Islamic Elegance",
  "Emerald Botanical Glasshouse",
  "Celestial Euphoria Birthday",
  "Terracotta Sunset Boho",
  "Y2K Pop Birthday Fiesta",
];
...
const CATEGORY_CHOICES = [
  ...
  { value: "aqiqah", label: "Aqiqah & Khitan" },
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 flex flex-col gap-2">
      {toasts.map((t) => (
        <div
          key={t.id}
          className="bg-slate-950 text-white text-xs font-mono px-4 py-3 rounded-xl border border-white/10 shadow-xl"
        >
          {t.message}
        </div>
      ))}
...
export default function DashboardAdmin() {
  ...
  const [templateCategory, setTemplateCategory] = useState(CATEGORY_CHOICES[0].value);
  const [templatePrice, setTemplatePrice] = useState("99000");
  ...
  function handleAddOrder(e: React.FormEvent) {
    ...
    setClientName("");
    setClientPhone("");
    showToast(`🎉 Pesan baru untuk "${name}" berhasil dibuat & langsung berstatus LIVE!`);
  }
  ...
  function handleAddTemplate(e: React.FormEvent) {
    ...
    const category = CATEGORY_CHOICES.find((c) => c.value === templateCategory);
    const price = parseInt(templatePrice, 10) || 99000;

    const newTemplate: Template = {
      id: `custom-${Date.now()}`,
      title,
      categoryLabel: category?.label.toUpperCase() ?? templateCategory.toUpperCase(),
      price,
      rating: 5.0,
      reviewsCount: 1,
      badge: "NEW",
  ...
  function toggleOrderStatus(orderId: string) {
    setOrders((prev) =>
      prev.map((o) => {
        ...
        let nextStatus: WorkflowStatus = "Draft";
        if (o.workflowStatus === "Draft") nextStatus = "In Progress";
        else if (o.workflowStatus === "In Progress") nextStatus = "Live";
        const nextPayment = nextStatus === "Live" ? "Lunas" : o.paymentStatus;
        return { ...o, workflowStatus: nextStatus, paymentStatus: nextPayment };
      })
    );
    const order = orders.find((o) => o.orderId === orderId);
    if (order) showToast(`🔄 Status pesanan ${order.orderId} diubah.`);
  }

  function contactClientWa(phone: string, name: string, slug: string) {
  ...
  function handleSaveSystemSettings(e: React.FormEvent) {
    e.preventDefault();
    // TODO: kirim ke API Laravel, lihat catatan di bawah
  ...
  const NAV_TABS: { id: TabId; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "settings", label: "Pengaturan Bisnis", icon: <Settings className="w-3.5 h-3.5" /> },
  ];
    ...
    return (
      <div className="bg-[#0a192f] text-white selection:bg-yellow-400 selection:text-black min-h-screen flex items-center justify-center p-4">
        <div className="pop-card bg-white text-slate-900 rounded-3xl p-8 max-w-md w-full text-center shadow-2xl relative">
          ...
          <h2 className="font-display text-4xl text-slate-950">Dashboard Admin</h2>
          <p className="text-xs text-slate-600 mb-6 leading-relaxed">
            Masuk dengan akun administrator untuk mengelola bisnis Bluevite.id.
          </p>
          ...
          <form onSubmit={handleLogin} className="space-y-4 text-left">
            ...
            >
              {loggingIn ? "Memproses..." : "Masuk sebagai Admin"}
            </button>
          </form>
        </div>
      </div>
  ...
  return (
    <div className="bg-[#0a192f] text-white selection:bg-yellow-400 selection:text-black min-h-screen">
      ...
      <header className="bg-[#0f2042] border-b-2 border-black sticky top-0 z-40 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/" className="flex items-center gap-2.5 group">
              ...
              <div>
                <span className="font-impact text-xl tracking-wider text-white">
                  CONCRETE BLOOM<span className="text-yellow-400">.ADMIN</span>
                </span>
            ...
            <div className="hidden md:flex items-center gap-2 bg-yellow-400 text-black border-2 border-black px-3 py-1 rounded-full text-xs font-mono font-bold">
          ...
          <div className="flex items-center gap-3">
            <Link href="/" className="btn-pop-dark py-2 px-3 rounded-xl text-xs font-bold font-mono">
              ...
              <span className="hidden sm:inline">Lihat Web Penjualan</span>
            </Link>
            <Link
            ...
            >
              ...
              <span>Portal Klien Pengantin</span>
            </Link>
            <button
              onClick={handleLogoutAdmin}
              className="btn-pop-dark py-2 px-3 rounded-xl text-xs font-bold font-mono text-rose-300 hover:text-white"
            >
              <span>Keluar</span>
            </button>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* KPI ROW (selalu tampil di atas, semua tab) */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8 text-left">
          ...
          <KpiCard label="TOTAL TAMU TERKELOLA" value="48.920" sub="Dalam sistem dashboard" icon={<Users className="w-4 h-4 text-purple-600" />} valueClass="text-purple-600" />
          <KpiCard label="CONVERSION RATE" value="18.4%" sub="Dari kunjungan web" icon={<TrendingUp className="w-4 h-4 text-amber-500" />} valueClass="text-amber-600" />
        </div>
          ...
          <div className="space-y-6">
            <div className="pop-card bg-white text-slate-900 rounded-3xl p-6 sm:p-8 text-left shadow-xl">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-200 pb-4 mb-6">
                <div>
                  ...
                  <p className="text-xs text-slate-600">
                    Kelola status pesanan, pembayaran, dan aktivasi link undangan klien.
                  </p>
                </div>
              ...
              <form onSubmit={handleAddOrder} className="bg-blue-50 border-2 border-blue-200 p-4 rounded-2xl mb-6">
                <span className="text-xs font-bold text-blue-900 block mb-3">➕ Buat Pesanan Baru Manual:</span>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {orders.map((o) => (
                      <tr key={o.orderId} className="hover:bg-slate-50 transition text-left">
                        ...
                        <td className="p-3.5">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${paymentBadgeClass(o.paymentStatus)}`}>
                            {o.paymentStatus}
                          </span>
                        </td>
                        <td className="p-3.5">
                        ...
                        <td className="p-3.5">
                          <div className="flex items-center gap-1.5">
                            <button
                              onClick={() => toggleOrderStatus(o.orderId)}
                              title="Ganti Status"
                            ...
                            <button
                              onClick={() => contactClientWa(o.phone, o.clientName, o.slug)}
        ...
        {activeTab === "templates" && (
          <div className="pop-card bg-white text-slate-900 rounded-3xl p-6 sm:p-8 text-left shadow-xl">
            ...
            <p className="text-xs text-slate-600 mb-6">Tambah tema baru atau ubah harga dan badge promosi.</p>

            <form onSubmit={handleAddTemplate} className="bg-yellow-50 border-2 border-yellow-300 p-4 rounded-2xl mb-6">
              <span className="text-xs font-bold text-amber-900 block mb-3">➕ Tambah Template Baru ke Katalog:</span>
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <input
                  type="text"
                  placeholder="Nama Tema Undangan"
                  required
                  value={templateTitle}
                ...
                <tbody className="divide-y divide-slate-100">
                  {templates.map((t) => (
                    <tr key={t.id} className="hover:bg-slate-50 transition text-left">
                      ...
                      <td className="p-3.5">
                        <button
                          onClick={() => showToast("✏️ Mode edit tema diaktifkan.")}
                          className="btn-pop-dark py-1 px-2.5 rounded-lg text-[11px] font-bold"
                        >
                          Edit
                        </button>
                      </td>
        ...
        {activeTab === "domains" && (
          <div className="pop-card bg-white text-slate-900 rounded-3xl p-6 sm:p-8 text-left shadow-xl">
            ...
            <p className="text-xs text-slate-600 mb-6">
              Pantau status propagasi domain klien dan sertifikat SSL otomatis.
            </p>
        ...
        {/* ============================== TAB: SETTINGS ============================== */}
        {activeTab === "settings" && (
          <div className="max-w-3xl pop-card bg-white text-slate-900 rounded-3xl p-6 sm:p-8 text-left shadow-xl">
            <h3 className="font-display text-3xl text-slate-950 mb-1">Pengaturan Sistem &amp; Bisnis</h3>
            <p className="text-xs text-slate-600 mb-6">
              Konfigurasi nomor kontak admin WhatsApp dan rekening penerimaan pembayaran.
            </p>

            <form onSubmit={handleSaveSystemSettings} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Nomor WhatsApp Admin Official</label>
                <input
                  type="text"
                  defaultValue="6282223551205"
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2 text-xs font-mono font-medium focus:ring-2 focus:ring-blue-600 focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Rekening Pembayaran Utama (BCA)</label>
                <input
                  type="text"
                  defaultValue="8830192841 - a.n PT Bluevite Digital Media"
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2 text-xs font-mono font-medium focus:ring-2 focus:ring-blue-600 focus:outline-none"
                />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1">Email Notifikasi Pesanan</label>
                <input
                  type="email"
                  defaultValue="orders@bluevite.id"
                  className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2 text-xs font-mono font-medium focus:ring-2 focus:ring-blue-600 focus:outline-none"
                />
              </div>
...
/**
 * CATATAN PENTING — LOGIN ADMIN (security.js + db.js lama):
 *
 * Halaman ini SENGAJA belum saya sambungkan ke sistem login apa pun. Di versi HTML
 * lama, login admin dicek di browser (client-side) dengan password hardcode
 * "admin123" di db.js — siapa pun bisa baca itu lewat DevTools. Ini TIDAK boleh
 * dibawa ke versi production.
 *
 * Cara yang benar nanti (pakai Laravel, bukan Next.js):
 * 1. Install Laravel Breeze/Fortify/Sanctum untuk sistem auth resmi
 *    (password di-hash dengan bcrypt di server, bukan SHA-256 di browser)
 * 2. Buat middleware Laravel yang melindungi route /api/admin/*
 * 3. Di Next.js, halaman /dashboard-admin dicek dulu via API login (misal
 *    POST /api/login), simpan token di httpOnly cookie (bukan localStorage,
 *    supaya tidak bisa dicuri lewat XSS)
 * 4. Middleware Next.js (middleware.ts) redirect ke halaman login kalau belum
 *    ada token valid, sebelum halaman dashboard-admin dirender
 *
 * CATATAN DATA (sama seperti dashboard-pembeli):
 * orders, templates, domains di atas masih hardcode di file ini. Nanti tinggal
 * fetch dari endpoint Laravel (Route::get('/api/orders', ...) dst.) dan ganti
 * useState(INITIAL_ORDERS) jadi useState([]) + useEffect(() => fetch(...), []).
 */